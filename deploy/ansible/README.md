...
Control-node logging is enabled in `ansible.cfg`:

```ini
log_path = /root/artifacts/ansible/ansible.log
display_args_to_stdout = False
no_target_syslog = True
```

Keep `display_args_to_stdout` disabled for normal runs because API tokens and
other sensitive values can appear in task arguments. Use task-level
`no_log: true` for any secret-bearing task.
