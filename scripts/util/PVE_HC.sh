...
if command -v zpool >/dev/null 2>&1; then
  ...
  if [[ -z "${arc_runtime:-}" || "$arc_runtime" = "0" ]]; then echo "WARN: ARC runtime cap not set (unbounded)." >> "$OUT"; warn=$((warn+1)); fi
  if [[ -z "${arc_persist:-}" || "$arc_persist" = "0" ]]; then echo "WARN: ARC persistent cap not set in /etc/modprobe.d/zfs.conf." >> "$OUT"; warn=$((warn+1)); fi
...
if command -v qm >/dev/null 2>&1; then
  ...
  while read -r VMID NAME _ _ _ _; do
...
if command -v pct >/dev/null 2>&1; then
  ...
  while read -r CTID _ _ NAME _; do
